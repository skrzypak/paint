...
void Shapes::Trapeze::update(const sf::Vector2i& curr, ShapeProperities* p)
{
    ...
    *_texture = (*p->texture);
    ...
    _convex->setPoint(2, sf::Vector2f(static_cast<float>(curr.x - len), static_cast<float>(_originPixel.y)));
    _convex->setPoint(3, sf::Vector2f(static_cast<float>(curr.x), static_cast<float>(curr.y)));
}